...
import { Calendar, MapPin, Users, ToggleLeft, ToggleRight, Settings, Award } from 'lucide-react';
...
export default function EventsManager() {
  ...
  const [prizeManagerEvent, setPrizeManagerEvent] = useState<{ id: string; title: string } | null>(null);
  ...
  return (
    <div>
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-piano-wine">Events Management</h2>
            ...
            <div className="flex items-start justify-between">
              <div className="flex-1">
                <div className="flex items-center mb-2">
                  <h3 className="text-lg font-semibold text-piano-wine">{event.title}</h3>
                  <span
                    className={`ml-3 px-2 py-1 text-xs font-medium rounded-full ${getStatusColor(event.status)}`}
                  >
                    {event.status}
                  </span>
                </div>
                ...
                <div className="mt-4 flex gap-2">
                  ...
                  <button
                    onClick={() => setPrizeManagerEvent({ id: event.id, title: event.title })}
                    className="inline-flex items-center px-3 py-1.5 text-sm font-medium text-piano-wine bg-piano-cream hover:bg-piano-gold/20 rounded-lg transition-colors duration-200"
                  >
      ...
      {events.length === 0 && (
        <div className="text-center py-12">
          ...
          <h3 className="mt-2 text-sm font-medium text-piano-wine">No events found</h3>
